...
public class ActivityPartakeImpl extends BaseActivityPartake {
    ...
    protected Result checkActivityBill(PartakeReq partake, ActivityBillVO bill) {
        ...
        // 校验：个人库存 - 个人活动剩余可领取次数
        if (bill.getUserTakeLeftCount() <= 0) {   //存在空指针情况，用户还没领取过，查到为null
        ...
        try {
            ...
            return transactionTemplate.execute(status -> {
                try {
                    ...
                    //写入领取活动信息
                    userTakeActivityRepository.takeActivity(bill.getActivityId(), bill.getActivityName(), bill.getStrategyId(), bill.getState(), bill.getTakeCount(), bill.getUserTakeLeftCount(), partake.getuId(), partake.getPartakeDate(), takeId);
